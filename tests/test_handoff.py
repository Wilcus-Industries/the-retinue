"""Tests for the merge reap (issue #12).

On a ``pull_request`` closed+merged signal (the human merged the PR), the retinue closes
the PR's owned issues, then reaps the parent: it closes the parent IFF every non-``hitl``
child issue is closed. An open ``hitl`` child does not block the reap; an open non-``hitl``
child does. Every gh touch is faked or injected.
"""
